...
feature -- Factory (Set)
	...
	some_immediate_set_a: MUTABLE_SET [A, EQ]
	...
	same_set_a (s: STS_SET [A, EQ]): like some_set_a
		...
		do
			...
			when 0 then
				Result := s
			when 1 then
				if attached {MUTABLE_SET [A, EQ]} s as effective_s then
...
----						suffixed_o: tmp_result.as_tuple.suffix (0).terms.is_empty
					...
					loop
						...
						tmp_result := tmp_result & same_object_a (l_s.any)
						l_s := l_s.others
					variant
...
--				Result := some_reference_sets_a
	...
	some_immediate_effecive_set_references_a: MUTABLE_SET [STS_SET [A, EQ], STS_REFERENCE_EQUALITY [STS_SET [A, EQ]]]
		...
		do
			check
				ss: attached {MUTABLE_SET [STS_SET [A, EQ], STS_REFERENCE_EQUALITY [STS_SET [A, EQ]]]} some_immediate_instance (
							agent: MUTABLE_SET [STS_SET [A, EQ], STS_REFERENCE_EQUALITY [STS_SET [A, EQ]]]
								do
									across
			...
			monomorphic: Result.generating_type ~ {detachable MUTABLE_SET [STS_SET [A, EQ], STS_REFERENCE_EQUALITY [STS_SET [A, EQ]]]}
		end
...
--				Result := some_reference_sets_a
	...
	some_immediate_effecive_set_standard_objects_a: MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_STANDARD_EQUALITY [STS_SET [A, EQ]]]
		...
		do
			check
				ss: attached {MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_STANDARD_EQUALITY [STS_SET [A, EQ]]]} some_immediate_instance (
							agent: MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_STANDARD_EQUALITY [STS_SET [A, EQ]]]
								do
									across
										1 |..| some_count.as_integer_32 as i
									from
			...
			monomorphic: Result.generating_type ~ {detachable MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_STANDARD_EQUALITY [STS_SET [A, EQ]]]}
		end
...
--				Result := some_reference_sets_a
	...
	some_immediate_effecive_set_objects_a: MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_EQUALITY [STS_SET [A, EQ]]]
			-- Randomly-fetched monomorphic set of sets of elements like {A}, and whose (member) sets are compared by their object values.
		do
			check
				ss: attached {MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_EQUALITY [STS_SET [A, EQ]]]} some_immediate_instance (
							agent: MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_EQUALITY [STS_SET [A, EQ]]]
								do
									across
										1 |..| some_count.as_integer_32 as i
									from
			...
			monomorphic: Result.generating_type ~ {detachable MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_EQUALITY [STS_SET [A, EQ]]]}
		end
...
--				Result := some_reference_sets_a
	...
	some_immediate_effecive_set_deep_objects_a: MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_DEEP_EQUALITY [STS_SET [A, EQ]]]
		...
		do
			check
				ss: attached {MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_DEEP_EQUALITY [STS_SET [A, EQ]]]} some_immediate_instance (
							agent: MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_DEEP_EQUALITY [STS_SET [A, EQ]]]
								do
									across
										1 |..| some_count.as_integer_32 as i
									from
			...
			monomorphic: Result.generating_type ~ {detachable MUTABLE_SET [STS_SET [A, EQ], STS_OBJECT_DEEP_EQUALITY [STS_SET [A, EQ]]]}
		end
