...
feature -- Test routines (Quality)
	...
	test_is_negative_infinity
		...
		do
			...
			assert ("not (- NaN).is_negative_infinity", not (- Nan).is_negative_infinity)
			assert ("not (- NaN).is_negative_infinity ok", is_negative_infinity_ok (- Nan, some_real_number))

			assert ("not NaN.is_negative_infinity", not Nan.is_negative_infinity)
			assert ("not NaN.is_negative_infinity ok", is_negative_infinity_ok (Nan, some_real_number))

			assert ("Negative_infinity.is_negative_infinity", Negative_infinity.is_negative_infinity)
			assert ("Negative_infinity.is_negative_infinity ok", is_negative_infinity_ok (Negative_infinity, some_real_number))

			assert ("(- Positive_infinity).is_negative_infinity", (- Positive_infinity).is_negative_infinity)
			assert ("(- Positive_infinity).is_negative_infinity ok", is_negative_infinity_ok ((- Positive_infinity), some_real_number))

			assert ("not Positive_infinity.is_negative_infinity", not Positive_infinity.is_negative_infinity)
			assert ("not Positive_infinity.is_negative_infinity ok", is_negative_infinity_ok (Positive_infinity, some_real_number))
	...
	test_is_finite
			-- Test {STI_REAL_NUMBER}.is_finite.
		note
			testing: "covers/{STI_REAL_NUMBER}.is_finite"
		local
			x: like real_number_to_be_tested
			l_check: BOOLEAN
		do
			...
			assert ("x.is_finite", x.is_finite)

			x := real_number_to_be_tested
			l_check := {ISE_RUNTIME}.check_assert (False)
			x := x / Zero
			l_check := {ISE_RUNTIME}.check_assert (l_check)
			assert ("not x.is_finite", not x.is_finite)

			assert ("not (- NaN).is_finite", not (- Nan).is_finite)
			assert ("not NaN.is_finite", not Nan.is_finite)
			assert ("not (- Negative_infinity).is_finite", not (- Negative_infinity).is_finite)
			assert ("not Negative_infinity.is_finite", not Negative_infinity.is_finite)
			assert ("not (- Positive_infinity).is_finite", not (- Positive_infinity).is_finite)
			assert ("not Positive_infinity.is_finite", not Positive_infinity.is_finite)
...
feature -- Test routines (Comparison)
	...
	test_is_less_equal
		...
		do
			...
			y := some_real_number
			assert ("-NaN ≤ y", - Nan ≤ y)
			assert ("-NaN ≤ y ok", is_less_equal_ok (- Nan, y, some_real_number))
			...
			assert ("-Infinity ≤ y", Negative_infinity ≤ y)
			assert ("-Infinity ≤ y ok", is_less_equal_ok (Negative_infinity, y, some_real_number))
			...
			assert ("not (-Infinity ≤ y)", not (Negative_infinity ≤ y))
			assert ("not (-Infinity ≤ y) ok", is_less_equal_ok (Negative_infinity, y, some_real_number))

			y := same_real_number (Positive_infinity)
			assert ("Infinity ≤ y", Positive_infinity ≤ y)
			assert ("Infinity ≤ y ok", is_less_equal_ok (Positive_infinity, y, some_real_number))
			...
			assert ("not (Infinity ≤ y)", not (Positive_infinity ≤ y))
			assert ("not (Infinity ≤ y) ok", is_less_equal_ok (Positive_infinity, y, some_real_number))
	...
	test_is_greater_equal
		...
		do
			...
			assert ("-Infinity ≥ y", Negative_infinity ≥ y)
			assert ("-Infinity ≥ y ok", is_greater_equal_ok (Negative_infinity, y, some_real_number))
			...
			assert ("not (-Infinity ≥ y)", not (Negative_infinity ≥ y))
			assert ("not (-Infinity ≥ y) ok", is_greater_equal_ok (Negative_infinity, y, some_real_number))

			y := some_real_number
			assert ("Infinity ≥ y", Positive_infinity ≥ y)
			assert ("Infinity ≥ y ok", is_greater_equal_ok (Positive_infinity, y, some_real_number))
